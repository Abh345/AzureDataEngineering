...
# MAGIC %md
# MAGIC ##This techq. is dependent on other tables where we already have INC. feature impl.
...
# MAGIC %md
# MAGIC ###Below techq is not depentdent on other tables .This is more meaningful 
...
spark.sql(f"""
              CREATE TABLE IF NOT EXISTS f1_presentation.calculated_race_results_optimized
              (
              race_year INT,
              team_name STRING,
              driver_id INT,
...
spark.sql(f"""
              MERGE INTO f1_presentation.calculated_race_results_optimized tgt  -- it is traget table so needs to be created upfront.
...
# MAGIC %sql
# MAGIC select count(*)  from f1_presentation.calculated_race_results_optimized 
