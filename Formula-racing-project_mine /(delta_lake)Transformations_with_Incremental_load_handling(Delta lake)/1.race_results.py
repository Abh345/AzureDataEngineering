...
races_df_1 = spark.read.format("delta").load(f"{processed_folder_path}/races") #full load 
...
final_df=results_df.join(race_circuit_joined_data,race_circuit_joined_data.race_id==results_df.race_id,'inner').join(drivers_df,drivers_df.driver_id==results_df.driver_id).join(constructors_df, results_df.constructor_id == constructors_df.constructor_id).select(races_df.race_id, races_df.race_year, races_df.race_name, races_df.race_date, circuits_df.circuit_location,drivers_df.driver_name,drivers_df.driver_number,drivers_df.driver_nationality,drivers_df.driver_id,results_df.grid,results_df.fastest_lap,results_df.race_time,results_df.points,constructors_df.team,results_df.position,results_df.file_date).withColumn("created_date", current_timestamp()).withColumnRenamed("result_file_date", "file_date")
